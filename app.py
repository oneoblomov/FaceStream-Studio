...
DEFAULT_LANGUAGE = next(iter(LANGUAGES), "English")

def get_lang():
    return LANGUAGES.get(st.session_state.get("language", DEFAULT_LANGUAGE), LANGUAGES.get(DEFAULT_LANGUAGE, {}))
...
def handle_media_stream(input_source, is_camera=True):
    analyzer = configure_analyzer(create_analyzer())
